...
def generate_markdown(prs_data, filename):
    ...
    with open(filename, "w", encoding="utf-8") as f:
        f.write(f"# team-mirai/policy リポジトリのPull Request分析\n\n")
        f.write(f"生成日時: {now}\n\n")
        f.write(f"## オープンなPull Requestの概要\n\n")
...
def generate_summary_markdown(prs_data, filename):
    ...
    with open(filename, "w", encoding="utf-8") as f:
        f.write(f"# team-mirai/policy リポジトリのPull Request概要\n\n")
        f.write(f"生成日時: {now}\n\n")
        f.write(f"## オープンなPull Requestの統計\n\n")
    ...
    print(f"サマリーMarkdownを 完了 に保存しました")
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="取得するPRの最大数 (デフォルト: 100, 0=全て)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="pr_analysis",
        help="出力ディレクトリ (デフォルト: pr_analysis)",
    )
...
def generate_issues_and_diffs_markdown(prs_data, filename):
    ...
    with open(filename, "w", encoding="utf-8") as f:
        f.write(
            f"# team-mirai/policy リポジトリのPull Request - Issues内容と変更差分\n\n"
        )
        f.write(f"生成日時: {now}\n\n")
        f.write(f"## 全PRのIssues内容と変更差分\n\n")
